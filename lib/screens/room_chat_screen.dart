...
class RoomChatScreen extends StatefulWidget {
  ...
  const RoomChatScreen({super.key, required this.contact});
...
class _RoomChatScreenState extends State<RoomChatScreen> {
  ...
  Future<void> _showCustomPathDialog(BuildContext context) async {
    ...
    appLogger.info('PathSelectionDialog returned: ${result?.length ?? 0} bytes, mounted: $mounted', tag: 'RoomChatScreen');

    if (result == null) {
      appLogger.info('PathSelectionDialog was cancelled or returned null', tag: 'RoomChatScreen');
      return;
    }

    if (!mounted) {
      appLogger.warn('Widget not mounted after dialog, cannot set path', tag: 'RoomChatScreen');
      return;
    }

    appLogger.info('Calling setPathOverride for ${widget.contact.name}', tag: 'RoomChatScreen');
    await connector.setPathOverride(
      widget.contact,
      pathLen: result.length,
      pathBytes: result,
    );
    appLogger.info('setPathOverride completed', tag: 'RoomChatScreen');
